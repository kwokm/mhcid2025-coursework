...
async function main() {    
    ...
    const storyCondensed = utils.extractJsonValues("Page Contents", JSON.parse(storyCreation)).join(" ");
    ...
    console.log("AI Prompt 4 - vocab translation & audio file finding Input: ", language + ": " + vocabularyWords);
    const finalOutput = await geminiPrompts.wordsToAudio(language + ": " + vocabularyWords);
    console.log("AI Prompt 4 - vocab translation & audio file finding Output: ", JSON.parse(finalOutput));
    console.log('\n');
