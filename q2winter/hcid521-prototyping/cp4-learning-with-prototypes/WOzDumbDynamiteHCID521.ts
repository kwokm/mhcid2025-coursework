...
function colorTransition(color1: number, color2: number) {
    ...
    while (true) {
        ...
        // rounded values
        strip.setAll(light.rgb(Math.round(r12 + (r22 - r12) * progress), Math.round(g12 + (g22 - g12) * progress), Math.round(b12 + (b22 - b12) * progress)))
        if (progress >= 1) {
            break;
        }
        basic.pause(5);
...
function transitionPixel(pixel: number, newColor: number) {
    ...
    while (true) {
        ...
        strip.setPixelColor(pixel, light.rgb(Math.round(r1 + (r2 - r1) * progress), Math.round(g1 + (g2 - g1) * progress), Math.round(b1 + (b2 - b1) * progress)))
        if (progress >= 1) {
            break;
        }
        basic.pause(5);
...
function setGradientColorRGB(r1: number, g1: number, b1: number, r2: number, g2: number, b2: number) {
...
control.runInParallel(function () { breathe(15, 30, 1, 1200) })
...
control.runInParallel(function () { breathe(15, 50, 1, 1100) })
...
basic.pause(3000)
control.runInParallel(function () { breathe(15, 50, 1, 1100) }) // bears
...
control.runInParallel(function () { createMovingGradient() })
